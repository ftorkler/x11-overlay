...
Color Color::fromAnsi(const std::string& ansi)
{
    int pos;

    // 24bit - ESC[38;2;[colorprofile];<r>;<g>;<b>m - foreground color
    if (ansi.rfind(PREFIX_24BIT_FG) == 0 && (pos = ansi.rfind(POSTIFX_M)) > PREFIX_24BIT_FG_LEN) {
        std::string colorCode = ansi.substr(PREFIX_8BIT_FG_LEN, pos - PREFIX_8BIT_FG_LEN);
        return _fromAnsi24bit(colorCode);
    }

    // 8bit - ESC[38;5;<code>m - foreground color
    if (ansi.rfind(PREFIX_8BIT_FG) == 0 && (pos = ansi.rfind(POSTIFX_M)) > PREFIX_8BIT_FG_LEN) {
        std::string colorCode = ansi.substr(PREFIX_8BIT_FG_LEN, pos - PREFIX_8BIT_FG_LEN);
        return _fromAnsi8bit(stoi(colorCode));
    }
...
Color Color::_fromAnsi24bit(std::string code)
{
...
Color Color::_fromAnsi8bit(int code)
{
    Color colors[] = {
        // standard colors
